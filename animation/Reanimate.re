...
//     let initialState = {height: 0., animation: SpringAnimation.create(0.)},
//     React.useEffect0(()=>({send}) => {
//       RemoteAction.subscribe(~send, rAction) |> ignore;
...
//     React.useEffect0(()=>({state: {State.randomAnimation: animation}, send}) => {
//       let callback =
//         (.) => {
//           let randomAction =
//             switch (Random.int(6)) {
...
//     let (state, dispatch) = React.useReducer(action, {act, items, randomAnimation} as state) =>
//       switch (action) {
...
//         {...state, items: Belt.List.keep(items, filter)});
...
//     let initialState = StartMessage;
//     let (state, dispatch) = React.useReducer(actionIsState, _) => actionIsState);
//     render: ({state, send}) =>
//       switch (state) {
//       | StartMessage =>
//         <div>
